...
class DemandRobustness:
    def __init__(self, scenario):
        ...
        self.results_dir = f"{os.getcwd()}/results/{self.scenario}"
        ...
        try:
            summary_df = pd.read_csv(f"{self.results_dir}/demand_robustness_stats.csv")
        except:
            summary_df.to_csv(f"{self.results_dir}/demand_robustness_stats.csv", index=False)
    ...
    def run_instance(self, idx):
        ...
        person_rank_df, occupancy_df = self.simulator.simulate()
        summary_stat = {}
