...
% rp: response rate
function [n] = invitation(solv, w, fs, s, mu, x1, x2, K, I, rp, x_lb1, x_lb2, mock)
    ...
    % w = 1; % weight on unused booking slots. w =1 corresponds to Most aggressive schedule
    size_d = size(mu);
    dd = size_d(1);
    T = (K-1)*I + dd;
    ...
    if mock == true
        [nv] = ones(K,1) * double((sum(x1)+sum(x2))/K);
    else
        if solv == 0
            if fs == 1
